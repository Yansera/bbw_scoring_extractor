...
def contextual_matching(filecsv, filename='', language='', semtab = False,
    ...
    if step2:
        for row in range(0, rows):  # We start here from row=1, because there are "col0" and "col1" in row=0
...
def postprocessing(cpa_list, cea_list, filelist=None, target_cpa=None, target_cea=None, target_cta=None, gui=False):
    ...
    bbw_candidate_scoring = copy.deepcopy(bbw_cea_sub)
    ...
    if filelist and not gui:
        ...
        try:
            ...
            pass
    return [bbw_cpa_sub, bbw_cea_sub, bbw_cta_sub, bbw_candidate_scoring]
...
def annotate(filecsv, filename='', language=''):
    ...
    [cpa, cea, nomatch] = contextual_matching(filecsv, filename, language,
                                              ...
                                              step6=True)
    [cpa_sub, cea_sub, cta_sub, cea_candidates] = postprocessing(cpa, cea, [filecsv], gui=True)

    return cea_candidates
    print(cea_sub)
